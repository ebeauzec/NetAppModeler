  ...
  if (unsupportedInfo) {
    ...
    if (unsupportedShelves.length > 0) {
      ...
      unsupportedShelves.forEach(shelf => {
        ...
        let reason = (profile.shelfErrors && profile.shelfErrors[shelf]) || (profile.shelfWarnings && profile.shelfWarnings[shelf]) || `Platform ${currentState.version.model} does not support ${name} shelves.`;
        let optionsTip = "";
        if (shelf === "ns224") {
