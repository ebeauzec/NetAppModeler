...
function populatePcieSlotDropdown() {
  ...
  slots.forEach(slot => {
    ...
    opt.textContent = `Slot ${slot.num} (${slot.type})` + (isOccupied ? " [Occupied]" : "");
...
function initStep4Inputs() {
  ...
  Array.from(shelfSelect.options).forEach(opt => {
    const val = opt.value;
    if (val === "none") return;
    const isSupported = profile.supportedShelves.includes(val);
    const errorMsg = profile.shelfErrors[val];
    
    opt.textContent = opt.textContent.replace(" (Unsupported)", "");
    if (errorMsg) {
