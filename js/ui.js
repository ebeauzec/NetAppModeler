...
function generateReport() {
  ...
  const phaseCount = [hasPhase1, hasPhase2, hasPhase3, hasPhase4, hasPhase5].filter(Boolean).length + 2;
  ...
  const depMatrixRows = [
    ...
    { phase: 'Phase 2', label: 'Firmware Updates', deps: ['Phase 0 complete', 'Phase 1 complete (if applicable)', 'No broken disks pending replacement', 'Cluster health: all nodes up'], show: hasPhase2 },
    { phase: 'Phase 3', label: 'ONTAP Configuration', deps: ['Phase 0 complete', 'Phase 2 complete (if applicable)', 'All LIFs operational', 'Storage failover enabled'], show: hasPhase3 },
    { phase: 'Phase 4', label: 'ONTAP Software Upgrade', deps: ['Phase 0 complete', 'Phase 1 complete (critical hardware clear)', 'Phase 2 complete (all firmware current)', 'Phase 3 complete (licenses/LIFs valid)', 'cluster image validate PASSED', 'HA enabled on all nodes', 'Auto-giveback enabled', upgradeNeeded && hops.length > 0 ? `Upgrade path verified: ${ontapVer} → ${hops.join(' → ')} → ${targetOntap}` : ''], show: hasPhase4 },
  ...
  const gate1to2 = hasPhase1 ? makeGate('Phase 1 (Critical Hardware)',
  ...
  <div class="report-section" style="margin-bottom:1.5rem;">
    ...
    ${gate1to2}
    ${hasPhase2 ? makePhase(2, '⚡', 'Firmware Updates (Disk → Shelf → SP/BMC → Switch)', '#f59e0b', phase2Steps, ['Phase 0: Maintenance window open', hasPhase1 ? 'Phase 1: Critical hardware resolved (no broken disks, HA enabled, cabling multipath)' : 'No critical hardware issues identified', 'Cluster fully operational — all nodes up']) : ''}
    ${gate2to3}
